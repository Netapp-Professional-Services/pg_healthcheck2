...
- All OpenSearch health checks
- Rules engine for severity scoring
- AI analysis (if configured)
- Trend shipping for historical tracking

Usage:
    python import_opensearch_diagnostic.py --diagnostic /path/to/api-diagnostics.zip --config config/opensearch.yaml

    # Or with an extracted directory
    python import_opensearch_diagnostic.py --diagnostic /path/to/api-diagnostics/ --config config/opensearch.yaml

    # With company name override
    python import_opensearch_diagnostic.py --diagnostic ./export.zip --config config/opensearch.yaml --company "Acme Corp"
...
class OpenSearchDiagnosticImporter:
    ...
    def run_import(self):
        ...
        """
        print(f"\n{'=' * 60}")
        print(f"OpenSearch Diagnostic Import Tool v{APP_VERSION}")
        print(f"{'=' * 60}")
...
Examples:
    # Import a zip file
    python import_opensearch_diagnostic.py --diagnostic ./api-diagnostics.zip --config config/opensearch.yaml

    # Import an extracted directory
    python import_opensearch_diagnostic.py --diagnostic ./api-diagnostics/ --config config/opensearch.yaml

    # With company name override
    python import_opensearch_diagnostic.py --diagnostic ./export.zip --config config/opensearch.yaml --company "Acme Corp"

    # Import with custom output name
    python import_opensearch_diagnostic.py --diagnostic ./diagnostic.zip --config config/opensearch.yaml --output client_report.adoc
        """
    )
    parser.add_argument(
        '--diagnostic', '-d',
    ...
    # Run import
    importer = OpenSearchDiagnosticImporter(
        args.config,
        args.diagnostic,
        args.report_config,
        args.company
