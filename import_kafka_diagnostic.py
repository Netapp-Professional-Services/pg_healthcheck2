#!/usr/bin/env python3
"""
Import and analyze Kafka Instacollector diagnostic exports.
...
- All Kafka health checks (topics, consumers, configuration, etc.)
- Rules engine for severity scoring
- AI analysis (if configured)
- Trend shipping for historical tracking

Usage:
    python import_kafka_diagnostic.py --diagnostic /path/to/InstaCollection.tar.gz --config config/kafka.yaml

    # Or with an extracted directory
    python import_kafka_diagnostic.py --diagnostic /path/to/instacollector_export/ --config config/kafka.yaml
...
class KafkaDiagnosticImporter:
    """Imports and analyzes Kafka Instacollector exports using the health check system."""

    def __init__(self, config_file: str, diagnostic_path: str, report_config_file: str = None, company_override: str = None):
        """
        Initialize the Kafka diagnostic importer.
        ...
        # Force db_type to kafka for plugin loading
        self.settings['db_type'] = 'kafka'

        # Load the Kafka plugin
        self.active_plugin = self._load_kafka_plugin()
        if not self.active_plugin:
            raise ValueError("Failed to load Kafka plugin")
        ...
        if report_config_file:
            ...
            instacollector_report = Path(__file__).parent / "plugins" / "kafka" / "reports" / "instacollector.py"
        ...
        self.connector = KafkaInstacollectorConnector(self.settings, str(self.diagnostic_path))

        # Update settings with cluster info for API submission
        if hasattr(self.connector, 'cluster_nodes') and self.connector.cluster_nodes:
            # Use first broker as host identifier
            first_broker = self.connector.cluster_nodes[0]
            self.settings['host'] = f"imported-{first_broker}"
    ...
    def _load_kafka_plugin(self):
        """Load the Kafka plugin."""
        try:
            from plugins.kafka import KafkaPlugin
            return KafkaPlugin()
        except ImportError as e:
            print(f"Failed to load Kafka plugin: {e}")
            return None
    ...
    def load_settings(self, config_file: str) -> dict:
        ...
        try:
            ...
            settings.setdefault('company_name', 'Kafka Import')
            ...
            settings.setdefault('db_type', 'kafka')
            settings.setdefault('host', 'imported')  # Will be updated with cluster info
            settings.setdefault('port', 9092)
            settings.setdefault('database', 'kafka')
    ...
    def run_import(self):
        ...
        """
        print(f"\n{'=' * 60}")
        print(f"Kafka Diagnostic Import Tool v{APP_VERSION}")
        print(f"{'=' * 60}")
        ...
        kafka_version = metadata.get('version', 'Unknown')
        kafka_mode = env_details.get('kafka_mode', 'unknown').upper()
        node_count = env_details.get('node_count', 'Unknown')

        print(f"\nDiagnostic Info:")
        print(f"  Collection Date: {collection_date}")
        print(f"  Kafka Version: {kafka_version}")
        print(f"  Mode: {kafka_mode}")
        print(f"  Brokers: {node_count}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Import and analyze Kafka Instacollector diagnostic exports',
...
Examples:
    ...
    python import_kafka_diagnostic.py --diagnostic ./instacollector_export/ --config config/kafka.yaml

    # With company name override
    python import_kafka_diagnostic.py --diagnostic ./export.tar.gz --config config/kafka.yaml --company "Acme Corp"

    # Import with custom output name
    python import_kafka_diagnostic.py --diagnostic ./diagnostic.tar.gz --config config/kafka.yaml --output client_report.adoc

    # Auto-detect most recent export in a directory
    python import_kafka_diagnostic.py --diagnostic /path/to/uploads/ --config config/kafka.yaml
        """
    )
    parser.add_argument(
        '--diagnostic', '-d',
    ...
    # Run import
    try:
        importer = KafkaDiagnosticImporter(
            args.config,
            args.diagnostic,
            args.report_config,
            args.company
